...
class Kax {
  constructor({ base, agentToken, storey, userAgent }) {
    this.base = base.replace(/\/+$/, ''); this.token = agentToken; this.storey = storey; this.ua = userAgent;
    this.room = storey ? `tower:${storey}` : '';
  }

  headers() {
    return { authorization: `Bearer ${this.token}`, 'content-type': 'application/json', 'user-agent': this.ua, accept: 'application/json' };
  }

  async post(path, body) {
    const r = await request('POST', `${this.base}${path}`, { headers: this.headers(), body: JSON.stringify(body), timeoutMs: 30000 });
   ...
   *  not there. Lines are capped; the city has its own cap too. */
  async say(text) {
  ...
  panel({ headline, lines, assetUrl, ctaRoomId }) {
    ...
    return this.post(`/tower/storey/${this.storey}/panel`, body);
  }
}
